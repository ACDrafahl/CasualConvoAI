# UPDATE: 
The bot works! See the changelog and development docs for current progress. There are still so many improvements that could be made, but it is now capable of speech-to-text-to-AI-to-text-to-speech conversation!
...
## Limitations
1. Searching exclusively for local LLMs greatly reduces the options available. GPT-4 is by far the easiest to implement, but the API is costly. Llama 2 can run free locally and is available for research and even commercial purposes, but it requires a substantial GPU, which breaks the consumer hardware requirement. One of the only options left is GPT4ALL, which is slower but runs on a CPU with lower RAM and storage requirements than most LLMs.
